...
from Celldivide.utils import get_r, get_distance, worker_receive_rate, area_receive_rate
...
class Area:
    ...
    # 判断坐标点是否在该网格区域内
    def is_inArea(self, pos_x, pos_y, bian=False):
        if bian is False:
    ...
    # 计算网格面积
    def get_square(self, is_latlng=False):
        if is_latlng is False:
            dx = self.pos_down[0] - self.pos_up[0]
            dy = self.pos_up[1] - self.pos_down[1]
        # 经纬度坐标
        else:
            dx = get_distance(self.pos_down[0], 0, self.pos_up[0], 0)   # 单位米
            dy = get_distance(0, self.pos_up[1], 0, self.pos_down[1])
        square = dx * dy
        return square
