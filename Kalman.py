...
def kf_trajectory(Lng, Lat):
    # 卡尔曼滤波
    t = len(Lng)
    kf_params_record = np.zeros((t, 4))
    kf_params_p = np.zeros((t, 4))

    kalman_filter_params = kf_init(Lng[0], Lat[0], 0, 0)
    for i in range(t):
        if i == 0:
            kalman_filter_params = kf_init(Lng[i], Lat[i], 0, 0)  # 初始化
        else:
            kalman_filter_params.z = np.transpose([Lng[i], Lat[i]])  # 设置当前时刻的观测位置
